...
router.get('/:id', (req, res) => {
  const db = getDb();
  try {
    const a = db.prepare('SELECT * FROM cpd_activities WHERE activity_id = ?').get(req.params.id);
...
router.post('/', (req, res) => {
  const db = getDb();
  try {
    const { cycle_id, registration_id, activity_title, activity_date, activity_type,
            delivery_format, units_claimed, is_clinical, is_verifiable, is_structured,
            topic_category, provider_name, units_multiplier, reflection_text } = req.body;
    ...
    // Resolve cycle_id and registration_id from active cycle if not provided
    ...
    const id = uuidv4();
    db.prepare(`INSERT INTO cpd_activities
...
router.put('/:id', (req, res) => {
  ...
  try {
    ...
    // Recalc units_awarded if units_claimed or multiplier changed
    ...
    db.prepare(`UPDATE cpd_activities SET ${updates.join(',')}, updated_at=? WHERE activity_id=?`).run(...vals);
    recalcCycle(db, a.cycle_id);
    res.json({ ok: true });
  } finally { db.close(); }
});
...
router.delete('/:id', (req, res) => {
  const db = getDb();
  try {
    const a = db.prepare('SELECT cycle_id FROM cpd_activities WHERE activity_id=?').get(req.params.id);
    if (!a) return res.status(404).json({ error: 'Not found' });
    db.prepare('DELETE FROM cpd_activities WHERE activity_id=?').run(req.params.id);
    recalcCycle(db, a.cycle_id);
    res.json({ ok: true });
  } finally { db.close(); }
});

// Recalculate cycle totals from activities
function recalcCycle(db, cycleId) {
  const totals = db.prepare(`
    ...
    FROM cpd_activities WHERE cycle_id=? AND status != 'rejected'`).get(cycleId);
  db.prepare(`UPDATE cpd_cycles SET
    units_completed=?, structured_completed=?, verifiable_completed=?, non_clinical_completed=?,
    updated_at=? WHERE cycle_id=?`).run(
